...
for shp_file in `ls ./data/gis/*/*.shp`; do
    TABLE_NAME="$(basename "$shp_file" .shp)"
    echo -n "$TABLE_NAME"
    # SRID: 2926
    # according to http://prj2epsg.org/search
    shp2pgsql -s 2926 "$shp_file" gis."$TABLE_NAME" >> "./data/gis/$TABLE_NAME.sql"
    # cat "./data/gis/$TABLE_NAME.sql" | psql -h "handelstaccato.homenet.org" -p 5432 -U gavin king_county
done
